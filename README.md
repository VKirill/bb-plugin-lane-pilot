...
- Plugin SDK 0.4.104 does not expose BB's selected UI language. Lane Pilot stores `auto` (default) or an explicit EN/RU override globally in plugin KV. In auto mode it recognizes the Russianizer only when its `data-footer-item="plugin:ru/toggle"` DOM action is present, then reads `bb-plugin-ru:enabled=on`; otherwise it uses a non-default document language (BB's `en` is treated as unknown), then `navigator.language`, and finally EN. A DOM observer and storage listener update mounted Lane Pilot surfaces when this signal changes. The explicit Lane Pilot override takes precedence. The old per-project `ui.language` control is removed from the Lane Pilot panel. The native BB locale is therefore a known SDK limitation rather than a synchronized setting.
