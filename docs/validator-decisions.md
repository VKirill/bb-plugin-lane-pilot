...
| Файл был грязным до spawn; SHA-256 содержимого изменился, а путь запрещён или не принадлежит `owns_paths` | `validation_failed` | [`pipeline.test.ts`](../tests/pipeline.test.ts): `rejects a pre-dirty never_touch file when its content changes again`; `rejects a pre-dirty file changed outside owns_paths` |
